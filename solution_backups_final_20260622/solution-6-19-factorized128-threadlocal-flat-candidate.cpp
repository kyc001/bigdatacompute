...
private:
    ...
    void init_thread_local_accumulators() {
        ...
        local_user_sum.assign(static_cast<std::size_t>(local_thread_count) * users, 0.0f);
        local_item_sum.assign(static_cast<std::size_t>(local_thread_count) * items, 0.0f);
        local_user_count.assign(static_cast<std::size_t>(local_thread_count) * users, 0);
        local_item_count.assign(static_cast<std::size_t>(local_thread_count) * items, 0);
    }

    void update_thread_local(const Rating* ratings, int n, float mean) {
        if (n < parallel_update_threshold) {
            float* const us = local_user_sum.data();
            float* const is = local_item_sum.data();
            int* const uc = local_user_count.data();
            int* const ic = local_item_count.data();
            int idx = 0;
            for (; idx + 1 < n; idx += 2) {
                const Rating& r0 = ratings[idx];
                const float residual0 = r0.rating - mean;
                is[r0.item] += residual0;
                ++ic[r0.item];
                us[r0.user] += residual0;
                ++uc[r0.user];

                const Rating& r1 = ratings[idx + 1];
                const float residual1 = r1.rating - mean;
                is[r1.item] += residual1;
                ++ic[r1.item];
            }
            if (idx < n) {
                const Rating& r = ratings[idx];
...
#pragma omp parallel
        {
            ...
            float* const us = local_user_sum.data() + static_cast<std::size_t>(tid) * users;
            float* const is = local_item_sum.data() + static_cast<std::size_t>(tid) * items;
            int* const uc = local_user_count.data() + static_cast<std::size_t>(tid) * users;
            int* const ic = local_item_count.data() + static_cast<std::size_t>(tid) * items;
...
#pragma omp for schedule(static)
        ...
        if ((n & 1) != 0) {
            ...
            const float residual = r.rating - mean;
            local_item_sum[r.item] += residual;
            ++local_item_count[r.item];
            local_user_sum[r.user] += residual;
            ++local_user_count[r.user];
        }
    }
...
#pragma omp critical(incremental_svd_score_rebuild)
        ...
        if (use_factorized_model) {
            for (int user = 0; user < users; ++user) {
                ...
                for (int tid = 0; tid < local_thread_count; ++tid) {
                    const std::size_t offset = static_cast<std::size_t>(tid) * users + user;
                    sum += local_user_sum[offset];
                    count += local_user_count[offset];
                }
            ...
            for (int item = 0; item < items; ++item) {
                ...
                int count = 0;
                for (int tid = 0; tid < local_thread_count; ++tid) {
                    const std::size_t offset = static_cast<std::size_t>(tid) * items + item;
                    sum += local_item_sum[offset];
                    count += local_item_count[offset];
                }
    ...
    static constexpr int parallel_update_threshold = 0;
    ...
    std::vector<float> local_user_sum;
    std::vector<float> local_item_sum;
    std::vector<int> local_user_count;
    std::vector<int> local_item_count;
