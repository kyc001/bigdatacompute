...
public:
    ...
    float predict(int user_id, int item_id) {
        const unsigned user = static_cast<unsigned>(user_id);
        const unsigned item = static_cast<unsigned>(item_id);
        if (__builtin_expect(user < static_cast<unsigned>(users) &&
                             item < static_cast<unsigned>(items) &&
                             has_updates && scores_ready && use_factorized_model, 1)) {
            return clip_score(user_score_data[user] + item_score_data[item]);
        }
        if (__builtin_expect(user >= static_cast<unsigned>(users) ||
                             item >= static_cast<unsigned>(items), 0)) {
            return global_mean;
        }
        if (__builtin_expect(!has_updates, 0)) {
            return global_mean;
        }
        if (__builtin_expect(!scores_ready, 0)) {
            ensure_scores_ready();
        }
        ...
        return clip_score(global_mean + user_score_data[user] + item_score_data[item]);
    }
