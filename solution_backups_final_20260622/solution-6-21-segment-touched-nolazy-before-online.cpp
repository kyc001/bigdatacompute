...
public:
    void load_base_model(float*, float*, int u_size, int i_size, int, float mean) {
        ...
        if (use_segment_model) {
            ...
            user_touch_epoch.assign(users, 0);
            item_touch_epoch.assign(items, 0);
            touched_users.reserve(batch_size_hint / user_stride + 1024);
    ...
    void update(const std::vector<Rating>& incremental_batch) {
        ...
        if (use_segment_model) {
            ...
            total_seen += n;
            has_updates = true;
            collect_touched_ids(ratings, n, base_phase);
...
#pragma omp for schedule(static)
    ...
    void begin_touch_epoch() {
        touched_users.clear();
        touched_items.clear();
        ++touch_epoch;
        if (touch_epoch == 0) {
            std::fill(user_touch_epoch.begin(), user_touch_epoch.end(), 0);
            std::fill(item_touch_epoch.begin(), item_touch_epoch.end(), 0);
            touch_epoch = 1;
        }
    }

    void collect_touched_ids(const Rating* ratings, int n, int base_phase) {
        begin_touch_epoch();
        const int epoch = touch_epoch;
        for (int idx = 0; idx < n; ++idx) {
            const int item = ratings[idx].item;
            if (item_touch_epoch[item] != epoch) {
                item_touch_epoch[item] = epoch;
                touched_items.push_back(item);
            }
        }
        const int user_start = (user_stride - base_phase) % user_stride;
        for (int idx = user_start; idx < n; idx += user_stride) {
            const int user = ratings[idx].user;
            if (user_touch_epoch[user] != epoch) {
                user_touch_epoch[user] = epoch;
    ...
    std::vector<int> user_touch_epoch, item_touch_epoch;
    std::vector<int> touched_users, touched_items;
    float* user_score_data = nullptr;
    float* item_score_data = nullptr;
    static constexpr int count_table_size = 65536;
    static constexpr int batch_size_hint = 100000;
};
