...
#pragma omp critical(incremental_svd_score_rebuild)
    ...
    void rebuild_scores() {
        if (use_factorized_model) {
            const float* user_sums[16];
            const int* user_counts[16];
            const float* item_sums[16];
            const int* item_counts[16];
            for (int tid = 0; tid < local_thread_count; ++tid) {
                user_sums[tid] = local_user_sum[tid].data();
                user_counts[tid] = local_user_count[tid].data();
                item_sums[tid] = local_item_sum[tid].data();
                item_counts[tid] = local_item_count[tid].data();
            }
            for (int user = 0; user < users; ++user) {
                float sum = 0.0f;
                int count = 0;
                for (int tid = 0; tid < local_thread_count; ++tid) {
                    sum += user_sums[tid][user];
                    count += user_counts[tid][user];
                }
            ...
            for (int item = 0; item < items; ++item) {
                ...
                int count = 0;
                for (int tid = 0; tid < local_thread_count; ++tid) {
                    sum += item_sums[tid][item];
                    count += item_counts[tid][item];
                }
