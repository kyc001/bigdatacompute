...
private:
    ...
    void build_count_tables() {
        user_count_table.resize(count_table_size + 1);
        item_count_table.resize(count_table_size + 1);
        for (int i = 0; i <= count_table_size; ++i) {
            const float count = static_cast<float>(i);
            const float log_count = std::log1p(count);
            user_count_table[i].score = coef[1] * log_count +
                                        coef[3] * log_count * log_count +
                                        coef[5] / std::sqrt(count + 1.0f);
            item_count_table[i].score = coef[2] * log_count +
                                        coef[4] * log_count * log_count +
                                        coef[6] / std::sqrt(count + 1.0f);
            user_count_table[i].weight = count > 0.0f ? coef[7] / (count + user_shrink) : 0.0f;
            item_count_table[i].weight = count > 0.0f ? coef[8] / (count + item_shrink) : 0.0f;
        }
    }
...
#pragma omp for schedule(static)
    ...
    float user_component_from(int user, float sum, int count) const {
        if (static_cast<unsigned>(count) <= static_cast<unsigned>(count_table_size)) {
            const CountParams params = user_count_table[count];
            return user_prior[user] + params.score + sum * params.weight;
        }
    ...
    float item_component_from(float sum, int count) const {
        if (static_cast<unsigned>(count) <= static_cast<unsigned>(count_table_size)) {
            const CountParams params = item_count_table[count];
            return params.score + sum * params.weight;
        }
        const float c = static_cast<float>(count);
        const float log_count = std::log1p(c);
...
#pragma omp critical(incremental_svd_score_rebuild)
    ...
    std::vector<CountParams> user_count_table;
    std::vector<CountParams> item_count_table;
