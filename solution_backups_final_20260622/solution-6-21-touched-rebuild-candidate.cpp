...
private:
    ...
    void initialize_zero_scores() {
        if (!use_factorized_model) {
            return;
        }
        const float user_zero = user_count_score_table[0];
        const float item_zero = item_count_score_table[0];
        for (int user = 0; user < users; ++user) {
            user_score[user] = user_prior[user] + user_zero;
        }
        for (int item = 0; item < items; ++item) {
            item_score[item] = item_zero;
        }
    }
    ...
    void init_thread_local_accumulators() {
        ...
        local_touched_users.clear();
        local_touched_items.clear();
        user_rebuild_epoch.assign(users, 0);
        item_rebuild_epoch.assign(items, 0);
        rebuild_epoch = 1;
        ...
        local_touched_users.resize(local_thread_count);
        local_touched_items.resize(local_thread_count);
    }
...
#pragma omp for schedule(static)
            for (int pair = 0; pair < pairs; ++pair) {
                ...
                const float residual0 = r0.rating - mean;
                if (ic[item0] == 0) {
                    local_touched_items[tid].push_back(item0);
                }
                is[item0] += residual0;
                ++ic[item0];
                if (uc[user0] == 0) {
                    local_touched_users[tid].push_back(user0);
                }
                us[user0] += residual0;
                ++uc[user0];
                ...
                const float residual1 = r1.rating - mean;
                if (ic[item1] == 0) {
                    local_touched_items[tid].push_back(item1);
                }
                is[item1] += residual1;
                ++ic[item1];
            }
        ...
        if ((n & 1) != 0) {
            ...
            const float residual = r.rating - mean;
            if (local_item_count[0][r.item] == 0) {
                local_touched_items[0].push_back(r.item);
            }
            local_item_sum[0][r.item] += residual;
            ++local_item_count[0][r.item];
            if (local_user_count[0][r.user] == 0) {
                local_touched_users[0].push_back(r.user);
            }
...
#pragma omp critical(incremental_svd_score_rebuild)
    ...
    void rebuild_scores() {
        if (use_factorized_model) {
            if (rebuild_epoch == 0x7fffffff) {
                std::fill(user_rebuild_epoch.begin(), user_rebuild_epoch.end(), 0);
                std::fill(item_rebuild_epoch.begin(), item_rebuild_epoch.end(), 0);
                rebuild_epoch = 1;
            }
            const int epoch = rebuild_epoch++;
            for (int tid = 0; tid < local_thread_count; ++tid) {
                const std::vector<int>& touched = local_touched_users[tid];
                for (int user : touched) {
                    if (user_rebuild_epoch[user] == epoch) {
                        continue;
                    }
                    user_rebuild_epoch[user] = epoch;
                    float sum = 0.0f;
                    int count = 0;
                    for (int src = 0; src < local_thread_count; ++src) {
                        sum += local_user_sum[src][user];
                        count += local_user_count[src][user];
                    }
                    user_score[user] = user_component_from(user, sum, count);
                }
            }
            for (int tid = 0; tid < local_thread_count; ++tid) {
                const std::vector<int>& touched = local_touched_items[tid];
                for (int item : touched) {
                    if (item_rebuild_epoch[item] == epoch) {
                        continue;
                    }
                    item_rebuild_epoch[item] = epoch;
                    float sum = 0.0f;
                    int count = 0;
                    for (int src = 0; src < local_thread_count; ++src) {
                        sum += local_item_sum[src][item];
                        count += local_item_count[src][item];
                    }
    ...
    std::vector<int> user_rebuild_epoch;
    std::vector<int> item_rebuild_epoch;
