...
private:
    ...
    void init_thread_local_accumulators() {
        ...
        local_user_accum.clear();
        local_item_accum.clear();
        if (local_thread_count <= 0) {
            return;
        }
        local_user_accum.assign(local_thread_count, std::vector<Accumulator>(users));
        local_item_accum.assign(local_thread_count, std::vector<Accumulator>(items));
    }
...
#pragma omp parallel
        {
            ...
            Accumulator* const ua = local_user_accum[tid].data();
            Accumulator* const ia = local_item_accum[tid].data();
...
#pragma omp for schedule(static)
        ...
        if ((n & 1) != 0) {
            ...
            const float residual = r.rating - mean;
            local_item_accum[0][r.item].sum += residual;
            ++local_item_accum[0][r.item].count;
            local_user_accum[0][r.user].sum += residual;
            ++local_user_accum[0][r.user].count;
        }
    }
...
#pragma omp critical(incremental_svd_score_rebuild)
        ...
        if (use_factorized_model) {
            for (int user = 0; user < users; ++user) {
                ...
                for (int tid = 0; tid < local_thread_count; ++tid) {
                    const Accumulator& accum = local_user_accum[tid][user];
                    sum += accum.sum;
                    count += accum.count;
                }
            ...
            for (int item = 0; item < items; ++item) {
                ...
                int count = 0;
                for (int tid = 0; tid < local_thread_count; ++tid) {
                    const Accumulator& accum = local_item_accum[tid][item];
                    sum += accum.sum;
                    count += accum.count;
                }
    ...
    std::vector<std::vector<Accumulator>> local_user_accum;
    std::vector<std::vector<Accumulator>> local_item_accum;
