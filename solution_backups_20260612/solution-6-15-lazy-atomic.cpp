...
public:
    void load_base_model(float*, float*, int u_size, int i_size, int, float mean) {
        ...
        total_seen = 0;
        has_updates = false;
        score_state.store(2, std::memory_order_relaxed);
    ...
    void update(const std::vector<Rating>& incremental_batch) {
        ...
        total_seen += n;
        has_updates = true;
        score_state.store(0, std::memory_order_release);
    }
    ...
    float predict(int user_id, int item_id) {
        ...
        if (score_state.load(std::memory_order_acquire) != 2) {
            ensure_scores_ready();
        }
...
#pragma omp critical(incremental_svd_score_rebuild)
        {
            if (score_state.load(std::memory_order_relaxed) != 2) {
                initialize_scores();
                score_state.store(2, std::memory_order_release);
            }
        }
    }
};
