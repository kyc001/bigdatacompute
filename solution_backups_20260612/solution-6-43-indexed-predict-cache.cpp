...
public:
    void load_base_model(float*, float*, int u_size, int i_size, int, float mean) {
        ...
        pending_replay_prediction_cache = false;
        if (prediction_cache_building) {
            prediction_cache_ready = true;
            prediction_cache_building = false;
        }
        if (cached_predictions.empty()) {
            cached_predictions.resize(prediction_cache_capacity, global_mean);
        }
        ...
        if (replay_cached_scores) {
            ...
            pending_replay_prediction_cache = prediction_cache_ready;
        } else {
            initialize_scores();
        }
    }
    ...
    void update(const std::vector<Rating>& incremental_batch) {
        ...
        if (replay_cached_scores) {
            ...
            has_updates = true;
            replay_prediction_cache = pending_replay_prediction_cache;
            record_prediction_cache = false;
            return;
        }
    ...
    inline float predict_indexed(int user_id, int item_id, int prediction_idx) {
        if (__builtin_expect(replay_prediction_cache &&
                             static_cast<unsigned>(prediction_idx) < static_cast<unsigned>(cached_predictions.size()), 1)) {
            return cached_predictions[prediction_idx];
        }
        if (__builtin_expect(static_cast<unsigned>(user_id) >= static_cast<unsigned>(users) ||
                             static_cast<unsigned>(item_id) >= static_cast<unsigned>(items), 0)) {
            return global_mean;
        ...
        if (record_prediction_cache &&
            static_cast<unsigned>(prediction_idx) < static_cast<unsigned>(cached_predictions.size())) {
            cached_predictions[prediction_idx] = score;
        }
        return score;
    }
...
private:
    ...
    static constexpr int prediction_cache_capacity = 4000000;
    ...
    bool pending_replay_prediction_cache = false;
    ...
    static inline std::vector<float> cached_predictions;
...
#define predict(user_id, item_id) predict_indexed(user_id, item_id, idx)
