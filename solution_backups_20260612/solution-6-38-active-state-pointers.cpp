...
public:
    void load_base_model(float*, float*, int u_size, int i_size, int, float mean) {
        ...
        neutral_user_score.assign(users, 0.0f);
        neutral_item_score.assign(items, global_mean);
        ...
        total_seen = 0;
        active_user_score = neutral_user_score.data();
        active_item_score = neutral_item_score.data();
    ...
    void update(const std::vector<Rating>& incremental_batch) {
        ...
        total_seen += n;
        refresh_scores();
        active_user_score = user_score.data();
        active_item_score = item_score.data();
    }
    ...
    inline float predict(int user_id, int item_id) {
        ...
        return clip_score(active_user_score[user_id] + active_item_score[item_id]);
    }
...
private:
    ...
    std::vector<float> neutral_user_score;
    std::vector<float> neutral_item_score;
    ...
    const float* active_user_score = nullptr;
    const float* active_item_score = nullptr;
