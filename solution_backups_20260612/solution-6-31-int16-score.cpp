...
public:
    void load_base_model(float*, float*, int u_size, int i_size, int, float mean) {
        ...
        user_score_q.assign(users, 0);
        item_score_q.assign(items, 0);
        user_score_data = user_score_q.data();
        item_score_data = item_score_q.data();
    ...
    void update(const std::vector<Rating>& incremental_batch) {
        ...
        const bool first_update = (total_seen == 0);
        total_seen += n;
        if (first_update) {
    ...
    float predict(int user_id, int item_id) {
        ...
        const int packed = static_cast<int>(user_score_data[user_id]) +
                           static_cast<int>(item_score_data[item_id]);
        return clip_score(global_mean + score_scale * static_cast<float>(packed));
    }
...
private:
    ...
    std::int16_t* user_score_data = nullptr;
    std::int16_t* item_score_data = nullptr;

    std::vector<float> user_sum;
    std::vector<float> item_sum;
    std::vector<int> user_count;
    std::vector<int> item_count;
    std::vector<std::int16_t> user_score_q;
    std::vector<std::int16_t> item_score_q;
    ...
    static constexpr float score_scale = 0.0001f;
    ...
    static std::int16_t quantize_score(float value) {
        const int rounded = static_cast<int>(value / score_scale + (value >= 0.0f ? 0.5f : -0.5f));
        if (rounded < -32768) {
    ...
    void initialize_scores() {
        for (int user = 0; user < users; ++user) {
            user_score_q[user] = quantize_score(user_component(user));
        }
        for (int item = 0; item < items; ++item) {
            item_score_q[item] = quantize_score(item_component(item) - global_mean);
        }
    }
