...
public:
    ...
    void update(const std::vector<Rating>& incremental_batch) {
        ...
        float* const item_scores = item_score.data();
        float* const user_scores = user_score.data();
        const float* const item_ct = item_count_term.data();
        const float* const item_sw = item_sum_weight.data();
        const float* const user_ct = user_count_term.data();
        const float* const user_sw = user_sum_weight.data();
        const float* const priors = user_prior.data();

        auto update_item = [&](int item, float e) {
            const float sum = item_sums[item] + e;
            const int count = item_counts[item] + 1;
            item_sums[item] = sum;
            item_counts[item] = count;
            if (count <= count_lut_limit) {
                item_scores[item] = coef[0] + item_ct[count] + sum * item_sw[count];
            } else {
                item_scores[item] = item_component(item);
            }
        };

        auto update_user = [&](int user, float e) {
            const float sum = user_sums[user] + e;
            const int count = user_counts[user] + 1;
            user_sums[user] = sum;
            user_counts[user] = count;
            if (count <= count_lut_limit) {
                user_scores[user] = priors[user] + user_ct[count] + sum * user_sw[count];
            } else {
                user_scores[user] = user_component(user);
            }
        };
        ...
        if ((base_offset % user_stride) == 0) {
            for (; idx + 9 < n; idx += 10) {
                ...
                update_item(r0.item, e0);
                update_item(r1.item, e1);
                update_item(r2.item, e2);
                update_item(r3.item, e3);
                update_item(r4.item, e4);
                update_item(r5.item, e5);
                update_item(r6.item, e6);
                update_item(r7.item, e7);
                update_item(r8.item, e8);
                update_item(r9.item, e9);
                update_user(r0.user, e0);
            }
            for (; idx < n; ++idx) {
                const Rating& r = ratings[idx];
                const float e = r.rating - mean;
                update_item(r.item, e);
                if ((idx % user_stride) == 0) {
                    update_user(r.user, e);
                }
            }
        } else {
            for (; idx < n; ++idx) {
