...
int main() {
    ...
    for (int run = 0; run < 5; ++run) {
        ...
        const double update_s = seconds_since(start);
        start = std::chrono::steady_clock::now();
        const float score = rmse(model, data.test);
        const double rmse_s = seconds_since(start);
        std::cout << "run " << run << " update " << update_s << " rmse_time " << rmse_s
                  << " total " << (update_s + rmse_s) << " rmse " << score << "\n";
    }
    return 0;
}
