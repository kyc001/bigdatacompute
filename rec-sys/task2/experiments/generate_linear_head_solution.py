...
WEIGHTS = Path("rec-sys/task2/experiments/generalized_linear_all_k1024.npz")
OUT = Path("rec-sys/task2/track1/solution.cpp")


def format_array(name, values, ctype="float"):
    vals = [float(v) for v in values]
    lines = [f"    static constexpr {ctype} {name}[embedding_dim] = {{"]
...
def main():
    ...
    K = int(w["K"])
    if K != 1024:
        raise ValueError(f"expected K=1024, got {K}")

    bias = float(w["bias"])
    user_w = w["user_w"].astype(np.float32)
    item_w = w["item_w"].astype(np.float32)
    p_mu = w["p_mu"].astype(np.float32)
    p_invstd = (1.0 / w["p_std"].astype(np.float32)).astype(np.float32)
    q_mu = w["q_mu"].astype(np.float32)
    q_invstd = (1.0 / w["q_std"].astype(np.float32)).astype(np.float32)
...
public:
    void load_base_model(float* user_matrix, float* item_matrix, int u_size, int i_size, int dim, float mean) {{
        ...
        user_static.assign(users, 0.0f);
        item_static.assign(items, 0.0f);
        user_score_data = user_score.data();
        item_score_data = item_score.data();
        build_embedding_offsets(user_matrix, item_matrix);
        build_count_tables();
        has_updates = false;
        scores_ready = true;
    }}
...
private:
    ...
    static constexpr int embedding_dim = 1024;

    static constexpr float intercept = {4.430029912552816 + bias:.9g}f;
...
{format_array("user_embedding_weight", user_w)}

{format_array("item_embedding_weight", item_w)}

{format_array("user_embedding_mean", p_mu)}

{format_array("item_embedding_mean", q_mu)}

{format_array("user_embedding_invstd", p_invstd)}

{format_array("item_embedding_invstd", q_invstd)}
    ...
    void build_embedding_offsets(const float* user_matrix, const float* item_matrix) {{
        const int d = std::min(latent_dim, embedding_dim);
        for (int user = 0; user < users; ++user) {{
            const float* const row = user_matrix + 1LL * user * latent_dim;
            float score = 0.0f;
            for (int k = 0; k < d; ++k) {{
                score += ((row[k] - user_embedding_mean[k]) * user_embedding_invstd[k]) *
                         user_embedding_weight[k];
            }}
        ...
        for (int item = 0; item < items; ++item) {{
            ...
            float score = 0.0f;
            for (int k = 0; k < d; ++k) {{
                score += ((row[k] - item_embedding_mean[k]) * item_embedding_invstd[k]) *
                         item_embedding_weight[k];
            }}
...
#pragma omp critical(incremental_svd_score_rebuild)
    ...
    std::vector<float> user_static;
    std::vector<float> item_static;
...
"""
    ...
    print(f"wrote {OUT} with generalized linear head")
