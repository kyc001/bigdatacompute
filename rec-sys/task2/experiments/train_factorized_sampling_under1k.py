...
class Config:
    ...
    def label(self):
        return (
            f"us{self.user_stride}_is{self.item_stride}_"
...
def load_arrays():
    ...
    return inc, test, users, items, mean


def sampled_stats(inc, users, items, mean, user_stride, item_stride):
    inc_u = inc[:, 0].astype(np.int32)
    inc_i = inc[:, 1].astype(np.int32)
    residual = inc[:, 2].astype(np.float32) - mean

    user_idx = np.arange(0, residual.shape[0], user_stride, dtype=np.int64)
    item_idx = np.arange(0, residual.shape[0], item_stride, dtype=np.int64)
    user_sum = np.bincount(inc_u[user_idx], weights=residual[user_idx], minlength=users).astype(np.float32)
    user_count = np.bincount(inc_u[user_idx], minlength=users).astype(np.float32)
    item_sum = np.bincount(inc_i[item_idx], weights=residual[item_idx], minlength=items).astype(np.float32)
    item_count = np.bincount(inc_i[item_idx], minlength=items).astype(np.float32)
...
def fit_base(x, y):
    xtx = x.T @ x
    xty = x.T @ y
    ridge = np.eye(x.shape[1], dtype=np.float64) * 1e-4
    coef = np.linalg.solve(xtx.astype(np.float64) + ridge, xty.astype(np.float64)).astype(np.float32)
    base = x @ coef
...
def main():
    ...
    inc, test, users, items, mean = load_arrays()
    configs = [
        Config(2, 2, seed=0),
        Config(3, 2, seed=0),
        Config(2, 3, seed=0),
    ...
    for cfg in configs:
        if cfg.total_params > PARAM_BUDGET:
            continue
        t0 = time.time()
        stats = sampled_stats(inc, users, items, mean, cfg.user_stride, cfg.item_stride)
        u, y, x = build_features(test, stats)
        coef, base, base_rmse = fit_base(x, y)
        print(f"\nCONFIG {cfg.label} params {cfg.total_params} base {base_rmse:.9f}", flush=True)
        ...
        if score < best[0]:
            ...
            np.savez(
                OUT_DIR / "factorized_sampling_under1k.npz",
                best_rmse=np.array(score, dtype=np.float32),
                user_stride=np.array(cfg.user_stride, dtype=np.int32),
                item_stride=np.array(cfg.item_stride, dtype=np.int32),
            ...
            print(f"NEW_BEST saved factorized_sampling_under1k.npz {score:.9f}", flush=True)
        del x, base
