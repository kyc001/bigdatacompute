...
MODEL = Path("rec-sys/task2/experiments/factorized_sampling_under1k.npz")
...
def main():
    ...
    low = int(model["low"])
    rank = int(model["rank"])
    user_stride = int(model["user_stride"])
    item_stride = int(model["item_stride"])
    if user_stride != 2 or item_stride != 2:
        raise ValueError("this generator currently emits the stride-2/stride-2 fast path")
...
{fmt_array(USER_SHRINKS, 10)}
    }};

    static constexpr float item_shrinks[10] = {{
{fmt_array(ITEM_SHRINKS, 10)}
    }};
...
"""
    ...
    float user_factor_prior(int user) const {
    ...
    void precompute_user_factor() {
        for (int user = 0; user < users; ++user) {
            user_factor_score[user] = user_factor_prior(user);
        }
    }
    ...
    void rebuild_scores() {
        ...
        if (use_factorized_model) {
            for (int user = 0; user < users; ++user) {
                ...
                user_factor_score[user] = user_factor_prior(user) + score;
            }
...
"""
    ...
    print(f"wrote {OUT} rmse {rmse:.9f} params {params} strides {user_stride}/{item_stride}")
