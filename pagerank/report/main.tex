...
使用 PyInstaller 将 \texttt{main.py} 打包为单文件可执行程序，打包参数保存在 \texttt{源码/compile-parameter.txt}。PyInstaller 会将 numpy、scipy 等依赖一并打包，生成的可执行文件无需额外安装 Python 环境即可运行。打包前先运行源码版本确认结果正常，打包后再用相同参数复测，比较源码版与可执行版的 Top-10 签名是否一致。

由于 PyInstaller 只能打包当前平台的可执行文件，无法交叉编译，因此提交包中提供 Windows 版 \texttt{main.exe}。Linux 或 macOS 用户可以使用源码运行，也可以在对应平台执行相同的 PyInstaller 命令生成本地可执行文件。提交包中附带 \texttt{Data.txt}，确保老师可以直接运行验证。
...
工程实现围绕统一入口、固定输出格式和可复现实验展开。\texttt{main.py} 提供稳定 CLI，\texttt{scripts/benchmark.py} 与 CSV 记录实验数据，PyInstaller 用于生成免安装可执行文件，\texttt{portable\_check/} 用于换机复核。跨平台方面，Windows 用户可直接运行 \texttt{main.exe}，Linux / macOS 用户可使用源码或在本地重新打包。
