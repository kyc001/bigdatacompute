...
- 和 B 的接口层对齐目前是“代码可用、文档仍有两个需要明确的点”：`dump_top_k` 没有显式 `id_map`、`blocks.py` 对 `dtype` 期望是类型对象而非 `np.dtype`。

### 待开始

- B 负责的最终 `csr_block` 提交版本打包与 PyInstaller 联调。
- C 负责的参数扫描、图表出图、报告总稿整合。
...
3. `baseline_dense.py` 只是实验基线，真实峰值内存约 422 MB，远超课程主提交通道的内存限制，不能作为最终方案。
...
| --- | --- | ---: | ---: | ---: | --- |
| E1 | dense | 422.645 | 0.456670 | 14 | 是 |
| E2 | csr | 40.615 | 0.455280 | 15 | 是 |

结论：

- CSR 相对 dense 的峰值内存下降约 `90.39%`。
...
| --- | ---: | ---: | ---: | --- |
| csr_block | 42.181 | 0.469851 | 15 | `75,8686,9678,5104,725,3257,468,7730,7175,5526` |

### E8 dead-end 策略对比（`beta=0.85, eps=1e-8`，同一份 `Data.txt`）

| 策略 | 时间 / s | 迭代轮数 | rank_sum | Top-10 与补偿法 Jaccard | 备注 |
| --- | ---: | ---: | ---: | ---: | --- |
| 质量补偿 | 0.3719 | 15 | 1.0000002 | 1.0000 | 主方案 |
| 忽略 dead-end | 1.2085 | 60 | 0.6020095 | 1.0000 | 概率质量明显泄漏 |
| 删除 dead-end | 0.5003 | 17 | 1.0000000 | 0.6667 | 先递归删掉了 1500 个节点，改变了原图语义 |
...
- 当前快照时间：`2026-04-25 16:43:40 UTC+8`
- 剩余：`19 天 19 小时 16 分 19 秒`
...
2. B 侧分块实现对 `dtype`、临时目录、`block/csr_block` 语义的假设还没有在接口文档里完全写死。
