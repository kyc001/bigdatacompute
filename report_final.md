# PageRank 高性能计算实验报告（阶段统稿）

> 本文件为 C 角色阶段统稿。A 的第 2、3 章与 E1/E2/E8 正式结果尚未接入；B 的第 4、6 章已纳入当前草稿口径。最终 PDF 前必须替换所有“待补”项。
...
本章应由 A 补充 PageRank 数学定义、随机游走模型、阻尼系数含义、马尔科夫链收敛解释，以及 dead-end 与 spider-trap 的处理原理。当前全队冻结公式为：
...
本章应由 A 补充 `load_graph / power_iteration / dump_top_k` 的最终实现说明，包括 CSR 构建、排序规则、收敛准则、Top-100 输出格式和复杂度分析。当前接口以 `INTERFACE.md` 为准。
...
实验 E3 显示，`K=4 / 8 / 16` 的平均峰值 RSS 分别约为 `35.16 MB`、`35.99 MB`、`34.97 MB`，平均时间分别约为 `0.376 s`、`0.423 s`、`0.457 s`。在当前数据规模下，继续增大 K 对内存降低不明显，反而增加块切换开销。

## 第 5 章 实验结果与分析

当前阶段实验数据均来自 CSV。E4 中，`csr` 平均峰值 RSS 约 `33.42 MB`、平均时间约 `0.492 s`；`csr_block` 平均峰值 RSS 约 `35.89 MB`、平均时间约 `0.418 s`。这说明当前小规模数据上分块版本并非 RSS 最低，但运行时间略优，且具有更好的扩展性。
...
本项目当前已经具备主流程、分块优化、benchmark、数据分析、参数扫描和阶段报告骨架。后续关键工作是 A 接入最终算法实现与实验结果，B 完成打包验证，C 重新生成最终图表并导出 PDF。
