...
更重要的是，数据集中存在 \texttt{1500} 个 dead-end 节点，占全部节点的 \texttt{15.00\%}。这些节点没有出边，如果在迭代中直接忽略，会导致概率质量逐渐流失，最终 PageRank 向量不再保持归一化。实现中采用统一的 dead-end 质量补偿公式：每一轮把 dangling mass 按 \texttt{beta * dangling\_mass / N} 均分回所有节点，再叠加随机跳转项 \texttt{(1 - beta) / N}。图~\ref{fig:deadend-strategy} 对比了不同 dead-end 处理策略，因此质量补偿是结果正确性的必要条件。
...
这一公式的含义非常直观：\texttt{v} 的权重由两部分组成。第一部分是“随机跳转”带来的均匀基础概率，保证任意节点都有被访问的机会；第二部分是来自所有入边源点的投票，每个源点 \texttt{u} 会把自己的权重平均分给它的所有出边终点。
...
dead-end 会直接影响概率质量守恒，必须统一处理。原因有三点：


\begin{enumerate}
\item 数据集里往往存在大量只被引用、不再向外链接的节点；
\item 一旦质量泄漏，后续所有 PageRank 值都会受到系统性偏差影响；
\item 若不同成员采用不同处理方式，实验和最终结果将无法对齐。
\end{enumerate}
...
从矩阵角度看，加入随机跳转之后，实际参与迭代的不是原始稀疏转移矩阵，而是带有均匀扰动的 Google Matrix。对于 \texttt{0 < beta < 1} 的情形，这个矩阵满足以下关键性质：
...
这意味着无论初始向量如何选择，只要按幂迭代反复更新，都将收敛到唯一的稳定分布。本实验固定采用
...
其背后的考虑是：\texttt{beta} 不能过小，否则图结构信息被过度稀释；也不能过大，否则对 spider-trap 更敏感、收敛更慢。\texttt{0.85} 是工程上常见、也足够稳定的折中值。\texttt{eps = 1e-8} 则使 Top-100 排名在多轮实验中保持稳定，兼顾了精度与时间。


\subsection{为什么选择 L1 范数作为停止条件}
...
\begin{enumerate}
\item L1 范数直接表示“全图总概率质量的移动量”，物理意义最清晰；
\item 它对大量小幅变化同样敏感，更适合比较两轮全局分布是否真正稳定；
\item L1 判据便于不同实验设置之间进行一致比较。
\end{enumerate}


在实现层面，L1 判据还便于与 \texttt{float32} 迭代搭配使用：只需要构造一个差值缓冲区，做原地减法、取绝对值和求和即可，不需要额外的平方或开方操作。


\subsection{本章小结}


综上，PageRank 的核心不是简单地“统计入边多少”，而是求一个在随机游走意义下自洽的稳态概率分布。dead-end 会造成概率质量流失，spider-trap 会造成局部质量滞留；前者必须通过质量补偿修正，后者则依赖 teleport 项被自然抑制。只要 Google Matrix 的构造正确，幂迭代就能稳定收敛到唯一解。第 3 章进一步说明如何将该公式落实为 CSR 稀疏实现。
...
这样做的直接收益是内存可控，并保证不同模块使用统一的数据约定。
...
这意味着图结构本身只需约 \texttt{664 KB} 量级的原始数组存储，已经远低于稠密矩阵方案。更重要的是，CSR 让 PageRank 的每轮迭代不再需要访问整张 $N \times N$ 矩阵，而只需顺着真实边做传播，从而避免了大量无意义的零乘加。


\subsection{分块策略：为什么不是普通切片}


仅有 CSR 还不够。若直接在内存中保留完整的 \texttt{r\_new}、反向邻接关系和辅助临时数组，Python 解释器、NumPy 运行时和若干中间缓冲区叠加后，仍可能让 RSS 峰值接近题目上限。因此，本项目的第二层优化不是“对数组做切片”，而是真分块：
...
这里按目标节点分块，而不是按源节点分块，原因是 PageRank 更新写入的是目标节点项 \texttt{r\_new[v]}。若按目标节点分块，则一个块内的所有边贡献都写回同一个局部连续区间，可以避免在多块之间频繁来回写入。
...
不同之处仅在于“对每个 \texttt{v} 的更新”被组织为“对每个目标块的更新”。因此，分块不会改变数值结果，只改变访问顺序与内存布局。
...
所有概率向量默认采用 \texttt{float32}，原因有三点。


第一，PageRank 本身是迭代近似问题，题目规定的停止阈值为 \texttt{1e-8}，并不要求达到双精度线性代数求解的极限精度。第二，\texttt{float32} 可将向量、缓冲区和临时块视图的空间消耗直接减半。第三，本项目的最终评价重点在于时间与 RSS 峰值，而不是极限数值精度。
...
这里 \texttt{float64} 的耗时略低，主要原因不是单次运算更快，而是双精度下收敛轮数减少。尽管如此，主提交版本仍建议固定为 \texttt{float32}，因为题目明确把内存控制作为核心约束，而 \texttt{float32} 在更大数据规模上会保留更稳妥的安全余量。
...
可以看到，本数据规模下继续增大 \texttt{K} 并不会显著降低 RSS，反而增加了块切换和文件访问开销。原因在于 \texttt{N = 10000}、\texttt{M = 150000} 本身并不大，真正占 RSS 的并不只是图数据，还包括 Python 解释器、NumPy 运行时和子进程开销。因此，过细分块的收益有限。
...
分块版本在时间上略有优势，但峰值 RSS 稍高。这说明在本数据规模上，分块更多体现为可扩展的内存控制手段，而不是小规模数据下的绝对内存最优。若数据集扩大，分块的收益会更明显，因为其活动工作集不再随全部边数线性增长。


\subsection{本章结论}


本项目的内存优化不是单点技巧，而是数据结构与执行策略的组合设计。CSR 解决“图怎么存”的问题，把空间复杂度从 $O(N^2)$ 压到 \texttt{O(N + M)}；分块 + \texttt{memmap} 解决“迭代时怎么访问”的问题，把整轮更新的活动工作集限制在当前块；\texttt{float32} 则进一步为主提交版本提供了更稳妥的内存余量。最终，在当前数据集上，主提交版本 \texttt{csr\_block + float32 + K=8} 的平均峰值 RSS 保持在约 \texttt{36 MB}，明显低于作业要求的 \texttt{80 MB} 上限，为最终实现提供了明确的工程基线。
...
综合实验结果，可以得到四点结论。第一，数据集稀疏性明显，CSR 是满足内存限制的必要选择。第二，dead-end 节点占比达到 \texttt{15.00\%}，必须按统一公式做质量补偿。第三，\texttt{beta} 对 Top-10 集合有一定影响，但核心高排名节点在 \texttt{0.80} 到 \texttt{0.90} 区间保持稳定。第四，收敛阈值越严格，迭代轮数明显增加，但 Top-10 排名在本数据集上保持稳定。
...
工程部分的目标是保证 PageRank 实现可以稳定运行、方便复现实验，并能按统一格式输出结果。主入口为 \texttt{main.py}，负责参数解析、算法调度、结果写入和运行摘要输出；图结构构建、分块迭代和实验统计分别封装在独立模块中，避免主程序承担过多职责。
...
\item 实验工具：\texttt{benchmark.py}、\texttt{sweep.py}、\texttt{plot.py}；
\item 配置与说明：\texttt{requirements.txt}、\texttt{compile-parameter.txt}、\texttt{README.md}。
\end{itemize}
...
该摘要便于 \texttt{benchmark.py} 自动收集运行时间、内存峰值、迭代轮数和 Top-10 签名。
...
最终可使用 PyInstaller 将 \texttt{main.py} 打包为单文件可执行程序。打包前先运行源码版本，确认 \texttt{Res.txt}、stdout JSON 和 Top-10 签名均正常；打包后再使用相同参数复测，比较源码版与可执行版的输出是否一致。该流程主要验证路径解析、依赖收集和结果格式，不改变算法逻辑。

\subsection{本章小结}

工程实现围绕统一入口、固定输出格式和可复现实验展开。\texttt{main.py} 提供稳定 CLI，\texttt{benchmark.py} 与 CSV 记录实验数据，PyInstaller 用于最终交付。这样既保证 PageRank 结果可复现，也便于比较 CSR、分块数和浮点精度等不同设置的影响。
...
\toprule
成员 & 代码模块 & 实验编号 & 报告章节 & 主要职责 \\
\midrule
A & \texttt{load\_graph}、\texttt{power\_iteration}、\texttt{dump\_top\_k} & E1、E2、E8 & 第 2、3 章 & PageRank 核心算法与正确性验证 \\
B & \texttt{build\_blocks}、\texttt{iterate\_by\_block}、\texttt{benchmark.py} & E3、E4、E7 & 第 4、6 章 & 分块优化、内存测试与工程集成 \\
C & \texttt{analyze\_dataset.py}、\texttt{sweep.py}、\texttt{plot.py} & E5、E6、数据集分析 & 第 1、5、7 章 & 数据分析、参数实验与报告整理 \\
\bottomrule
\end{tabularx}
\end{center}
...
第一，PageRank 的关键不只是入边统计，而是概率质量在全图上的稳定分布。dead-end 若处理不当会造成质量泄漏，spider-trap 若缺少 teleport 会造成局部吸收，因此两者必须在公式和实现中同时考虑。

第二，内存优化需要区分理论数组空间和实际 RSS。CSR 能显著降低图结构存储，但 Python 解释器、NumPy 运行时、临时缓冲区和文件映射也会影响峰值内存。报告中的内存结论应以 benchmark 实测为准。

第三，分块优化在小图上不一定带来最低 RSS，但它提供了更清晰的内存上界控制方式。当图规模扩大时，按目标节点分块可以避免一次性维护过大的更新工作集，因此具有更好的扩展意义。

总体而言，本项目完成了从 PageRank 理论公式到稀疏存储、分块迭代、实验验证和工程交付的完整流程。实验结果说明，CSR 与 Block Matrix 的结合能够在保持结果一致性的同时满足内存约束。
